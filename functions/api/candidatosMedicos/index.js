...
export async function onRequestGet(context) {
  ...
  const query = context.env.DB.prepare(`${candidateSelectSql(where)} ORDER BY created_at DESC LIMIT 500`).bind(...values);
...
export async function onRequestPost(context) {
  ...
  if (resume instanceof File && resume.size > 0) {
    ...
    await context.env.CANDIDATE_FILES.put(resumeKey, resume.stream(), {
  ...
  try {
    await context.env.DB.prepare(`INSERT INTO medical_candidates (
    ...
    if (resumeKey) await context.env.CANDIDATE_FILES.delete(resumeKey);
    console.error(error);
