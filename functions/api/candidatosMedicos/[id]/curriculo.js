...
export async function onRequestGet(context) {
  ...
  const candidate = await context.env.DB.prepare("SELECT resume_key, resume_name, resume_type FROM medical_candidates WHERE id = ? LIMIT 1")
    .bind(context.params.id).first();
  if (!candidate?.resume_key) return new Response("Currículo não encontrado.", { status: 404 });
  const file = await context.env.CANDIDATE_FILES.get(candidate.resume_key);
