...
async function findUser(db, email) {
  return db.prepare(`SELECT id, name, email, role, city, phone, permissions_json, active, password_hash, password_salt FROM users WHERE lower(email)=lower(?) LIMIT 1`).bind(email).first();
}
...
async function upsertInitialAdmin(context, db, email, password) {
  ...
  if (existing) {
    await db.prepare(`UPDATE users SET name=?, role='admin', city=?, password_hash=?, password_salt=?, permissions_json=?, active=1, updated_at=? WHERE id=?`)
      .bind(name, city, record.hash, record.salt, JSON.stringify(ADMIN_PERMISSIONS), now, existing.id).run();
  } else {
    await db.prepare(`INSERT INTO users (id,name,email,role,city,phone,password_hash,password_salt,permissions_json,active,created_at,updated_at) VALUES (?,?,?,'admin',?,'',?,?,?,1,?,?)`)
...
export async function onRequestPost(context) {
  ...
  try {
    ...
    if (initialEmail && initialPassword && email === initialEmail && password === initialPassword) {
      ...
      if (!valid || !user.active || String(user.role).toLowerCase() !== "admin") user = await upsertInitialAdmin(context, db, initialEmail, password);
    }

    stage = "verify";
    const validPassword = user && user.active && await verifyPassword(password, user.password_salt, user.password_hash).catch(() => false);
    if (!validPassword) return json({ error: "E-mail ou senha incorretos." }, 401);
    ...
    return json({ error: "Não foi possível concluir o login agora.", code: safeErrorCode(error, "AUTH_LOGIN_ERROR"), stage }, 500);
  }
}
