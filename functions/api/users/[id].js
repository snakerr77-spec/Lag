...
export async function onRequestPatch(context) {
  ...
  try {
    ...
    const current = await db.prepare(`SELECT id, name, email, role, city, phone, permissions_json,
      active, password_hash, password_salt
    ...
    let permissions = [];
    try { permissions = JSON.parse(current.permissions_json || "[]"); } catch {}

    if (adminEditing && Array.isArray(input.permissions)) {
      permissions = input.permissions.map(value => clean(value, 80)).filter(Boolean);
    }
    ...
    if (input.password) {
      ...
      const record = await createPasswordRecord(String(input.password));
    ...
    await db.prepare(`UPDATE users
      SET name = ?, email = ?, role = ?, city = ?, phone = ?, permissions_json = ?,
          active = ?, password_hash = ?, password_salt = ?, updated_at = ?
      WHERE id = ?`)
      .bind(
        name, email, role, city, phone, JSON.stringify(permissions),
        active, passwordHash, passwordSalt, now, targetId
      ).run();

    await audit(context.env, auth.user.id, "user_update", "user", targetId, {
      self, role, city, passwordChanged: Boolean(input.password)
    });

    const row = await db.prepare(`SELECT id, name, email, role, city, phone, permissions_json, active
      FROM users WHERE id = ? LIMIT 1`).bind(targetId).first();

    return json({ success: true, user: adminUser(row) });
  } catch (error) {
    console.error("user_update_error", error?.stack || error);
    return json({ error: "Não foi possível atualizar o usuário agora." }, 500);
  }
}
  ...
  try {
    ...
    if (isAdmin(target)) {
      ...
      if (Number(count?.total || 0) <= 1) {
        return json({ error: "É necessário manter ao menos um administrador ativo." }, 400);
      }
    }
