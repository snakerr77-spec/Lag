...
function selectedCity(request, session, requestedCity = "") {
  if (normalizeRole(session.user.role) !== "dev") return session.user.city;

  const requested = requestedCity || readCookie(request, "lag_active_city");
  if (!requested || requested === "Todas as cidades") return session.user.city;

  const allowed = new Set(["Cerquilho", "Tatuí", "Embu das Artes", "Itapeva"]);
  return allowed.has(requested) ? requested : session.user.city;
}

function scopeFor(session, requested, request, requestedCity = "") {
  ...
  return { type: "city", id: selectedCity(request, session, requestedCity) };
}
...
export async function onRequestPut(context) {
  ...
  const scope = scopeFor(session, input.scope, context.request, input.city);
...
export async function onRequestDelete(context) {
  ...
  const scope = scopeFor(session, url.searchParams.get("scope") || "city", context.request, url.searchParams.get("city") || "");
