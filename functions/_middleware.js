...
const PUBLIC_PREFIXES = ["/cadastro-medico/", "/laudos-medicos/portal-paciente/"];
...
async function loadState(env, user) {
  const result = await env.DB.prepare(`SELECT scope_type, state_key, value_text, updated_at
    FROM app_state
...
async function loadUsers(env, user) {
  ...
  const result = await env.DB.prepare(`SELECT id, name, email, role, city, phone, permissions_json, active
...
export async function onRequest(context) {
  ...
  const state = await loadState(context.env, session.user);
  const users = await loadUsers(context.env, session.user);
  const permissions = Object.fromEntries(users.map(user => [user.id, user.permissions || []]));
  const bootstrap = {
    enabled: true,
    user: session.user,
    users,
  ...
  const injection = `<script>window.__LAG_CLOUD__=${safeJson(bootstrap)};</script><script src="/shared/js/lag-cloud-sync.js?v=20260806"></script>`;
