...
export async function getSession(env, request) {
  ...
  if (!row || !row.active || new Date(row.expires_at).getTime() <= Date.now()) {
    if (row?.session_id) await db.prepare("DELETE FROM sessions WHERE id = ?").bind(row.session_id).run();
    return null;
  }
...
export async function audit(env, userId, action, targetType, targetId, details = {}) {
  try {
    await ensureCoreSchema(env);
    const db = getDb(env);
    await db.prepare(`INSERT INTO audit_logs (id, user_id, action, target_type, target_id, details_json, created_at)
      VALUES (?, ?, ?, ?, ?, ?, ?)`)
