(() => {
  ...
  function configureAccess() {
    ...
    const admin = isAdmin();
    const citySwitcher = admin || role === "dev";
    ...
    el.citySelect.disabled = !citySwitcher;
    el.reportCity.disabled = !(admin || role === "medico" || role === "dev");
    el.folderCity.disabled = !admin;
    el.categoryCity.disabled = !admin;
    el.cityLockIcon.hidden = citySwitcher;
    el.cityAccessText.textContent = citySwitcher
      ? `${settings.roleLabel(role)}: você pode alternar entre as quatro cidades.`
      : role === "medico"
        ? `Médico vinculado a ${state.city}. No cadastro do laudo você pode escolher a cidade de destino do PDF.`
        : `${settings.roleLabel(role)} vinculado(a) à unidade ${state.city}.`;
  ...
  function changeActiveCity() {
    ...
    if (!(isAdmin() || role === "dev")) {
      el.citySelect.value = state.city;
      return toast("Seu cargo não permite alterar a cidade.", "error");
    }
    state.city = normalizeCity(el.citySelect.value);
    state.categoryId = "";
    state.folderId = "";
    if (typeof settings.setActiveCity === "function") {
      const switched = settings.setActiveCity(state.city, role === "dev");
      if (switched && role === "dev") return;
    } else {
