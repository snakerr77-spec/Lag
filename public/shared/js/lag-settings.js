(() => {
  ...
  function ensureData() {
    if (window.__LAG_CLOUD__?.user) {
      ...
      storageSet(KEYS.activeCity, normalizeCity(cloudUser.unit || cloudUser.city || "Cerquilho"));
  ...
  function getActiveCity(user = getCurrentUser()) {
    return normalizeCity(user?.unit || user?.city || "Cerquilho");
  }
