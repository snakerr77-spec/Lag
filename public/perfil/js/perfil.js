(() => {
  ...
  async function saveTeamUser(event) {
    ...
    try {
      ...
      toast(editingId ? "Usuário atualizado com sucesso." : "Novo usuário criado com sucesso.");
      resetTeamForm();
      renderTeam();
      populatePermissionUsers();
      renderPermissionEditor();
  ...
  async function handleTeamAction(event) {
    ...
    if (button.dataset.teamAction === "edit") {
      ...
      el.teamForm?.scrollIntoView({ behavior: "smooth", block: "start" });
      el.teamName.focus();
      toast(`Editando ${user.name}. Altere os dados e clique em Salvar usuário.`);
      return;
    }
